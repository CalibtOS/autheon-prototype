...
- **Vehicle domain** (v2.7, client confirmation “Systemlogik Fahrzeugeingabe”, supersedes the v2.1 “important vehicle info” model) = **four explicit semantic categories** plus independent characteristics. **Never** one unstructured multi-select tag collection — that proposal was rejected. Use these exact terms:
...
- **Red licence plates** (v2.7) = a **DERIVED** requirement, never a manual selection and **never a recorded plate number**. Partners bring their own red plates; the number is irrelevant to order creation.
  ...
  **Removed in v2.7:** the manual “Red licence plates” option, the red-plate-number input, and the rule that hid/cleared the official licence plate when the vehicle was deregistered.
...
- **Legacy-data caveat:** historical records keep removed vehicle types **verbatim**; there is **no approved migration mapping**, so never remap SUV/Van/Classic-car/light-truck values (the only automatic rename is `PKW`/`Car` → `passenger_car`, a label rename of a retained option). Old manually captured red-plate numbers are preserved in a legacy audit field and never surface in active flows.
