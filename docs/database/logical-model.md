...
> **Status override:** Updated 2026-08-12 — **Backend sync pass + catch-up:** `schema.dbml` + this file remain the binding V1 relational source of truth. Originally verified against autheon-be `main` 824b1eea + PR #178; catch-up against `708c16be` closed the remaining gaps: `locations.alternate_contact_name` is now on the master row (and snapshotted onto `job_locations`); `master_data_change_requests.requester_note` and `user_notifications.severity` match DBML NOT NULL; TypeORM timestamp columns are annotated `timestamptz`. 38 tables here, 38 BE entity tables, same 38. Binary metadata only on `upload_assets`. Prefer DBML over thinner TypeORM shapes when they disagree.
...
| Finance                 | `job_financials` holds operational amounts (`revenue_amount`, `driver_offer_amount`, `expenses_amount`) plus optional net/gross and invoice/payment summary fields. BE columns are `currency_code` / `tax_rate` (API/domain still expose `vatRate`). Marketplace reads `driver_offer_amount` only. `jobs.settlement_state` remains the tour closeout state. `jobs.payment_status` / `invoice_received` / `invoice_number` are a denormalized board mirror written in lockstep with `job_financials`. | Whether advanced invoices/ledger/accounting integration become approved scope.                                   |
