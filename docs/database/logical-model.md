...
| Identity                 | `users`, `drivers`                                                                                                                       | Keycloak owns passwords and sessions. AUTHEON stores the local user record (`users`) linked by `keycloak_id` (BE physical column `identity_id`), with first/last name, **`account_access` + `invite_state` + `last_activity_at`**, roles, and email verification. Short-lived invite/password-reset **challenge hashes** may live on `users` (decision 1A). The driver business profile lives in `drivers` (`operational_access` + inactivity markers). Roles are persisted locally and provisioned to Keycloak on account invite. |
...
- Unique `users.email` and partial unique index on `users.keycloak_id where keycloak_id is not null` (BE physical column name today: `identity_id`).
...
Keycloak is the identity source of truth for authentication (passwords and sessions). Application services validate the Keycloak token, resolve `sub` to `users.keycloak_id` (BE: `identity_id`), and apply AUTHEON domain rules using the local `users` / `drivers` records.
